...
public class UMJDemo_PlayerController : MonoBehaviour {
	...
	{		
		...
		if ( RotateJoystick.JSK_TouchForce < MinTForceToMove ){
			PlayerRotation( MoveJoystick.JSK_Direction.y, MoveJoystick.JSK_Direction.x, MoveStrength );
			Inputs.caminarEsPresionado = true;
			JoyStick.presionActiva = true; 
		}else{
			Inputs.caminarEsPresionado = false;
	...
	private void PlayerRotation( float RotateFactor_Y, float RotateFactor_X, float RotateStrength )
	{
		...
		//NewRotation.eulerAngles = new Vector3 (NewRotation.eulerAngles.x,NewRotation.eulerAngles.y+45,NewRotation.eulerAngles.z);
		if (!FPSCamera) {
			Inputs.caminarEsPresionado = true;
			JoyStick.presionActiva = true; 
